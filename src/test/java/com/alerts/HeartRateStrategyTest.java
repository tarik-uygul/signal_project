...
class HeartRateStrategyTest {
    ...
    void testInsufficientData() {
        ...
        for (int i = 0; i < 10; i++) {
            patient.addRecord(1.0, "HeartRate", i * 1000L);
        }
    ...
    @Test
    void testAbnormalHeartRatePeak() {
        ...
        patient.addRecord(2.0, "HeartRate", 50000L);

        List<Alert> alerts = strategy.checkAlert(patient);
        assertEquals(1, alerts.size(), "An abnormal peak should trigger an alert.");
        assertEquals("Abnormal HeartRate Peak", alerts.get(0).getCondition());
    }

    @Test
    void testNormalHeartRateFluctuations() {
        // Add 60 records that fluctuate mildly around 1.0
        for (int i = 0; i < 60; i++) {
            double val = 1.0 + (i % 2 == 0 ? 0.1 : -0.1); // Flips between 1.1 and 0.9
            patient.addRecord(val, "HeartRate", i * 1000L);
        }
